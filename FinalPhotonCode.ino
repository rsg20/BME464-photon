...
unsigned long RecMillis=millis();
while(numPackets<MaxNumPackets &&(millis()-RecMillis)>(MaxNumPackets*32000)){
...
void gotData(const char *event, const char *data){
    ...
    int pwdEnd = pa-2;
    int paEnd = rc-2;
    int rdEnd = startrec-2;
    int StartRecEnd = startpace-2;
    int StartPaceEnd = stoppace-2;
    int StopPaceEnd = paramID-2;
    ...
    int MaxNumPackets=RecDurStr.toInt();
